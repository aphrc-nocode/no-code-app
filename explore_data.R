...
explore_data_server = function() {
	observeEvent(input$manage_data_apply, {
		...
		if (isTRUE(!is.null(input$dataset_id))) {
			rv_current$manage_data_title_transform=rv_current$manage_data_title_explore = renderUI({
				p(
					  HTML(
	...
	observe({
		if (NROW(rv_metadata$upload_logs)) {
			if (is.null(input$dataset_id)) {
				rv_current$manage_data_title_transform=rv_current$manage_data_title_explore = renderText(
